package com.example.clothlab.responsivespacing
...
class AdaptiveSpacingItemDecoration(private val size: Int, private val edgeEnabled: Boolean = false): ItemDecoration() {
    ...
    private fun makeGridSpacing(
        ...
        // Saved size
        val sizeBasedOnFirstColumn = if (isFirstColumn) sizeBasedOnEdge else 0
        val sizeBasedOnLastColumn = if (!isLastColumn) sizeBasedOnLastPosition else sizeBasedOnEdge
        val sizeBasedOnFirstRow = if (isFirstRow) sizeBasedOnEdge else 0
